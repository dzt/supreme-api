					...
					function (err, resp, html, rrr, body) {
						...
						if ($('option')) {
							...
							if (sizeOptionsAvailable.length > 0) {
                                // sizesAvailable = sizeOptionsAvailable.map((item) => {return item.size+":"+item.id})
						...
						var metadata = {
							...
							price: $('.price')[0].children[0].children[0].data.match(
								/(\d+)/
							)[0],
							image: image,
						...
						if ($('.styles').length > 0) {
							...
							for (li in styles) {
								for (a in styles[li].children) {
									if (
										styles[li].children[a].attribs['data-style-name'] ==
										metadata.style
									) {
										metadata.images.push(
											'https:' +
												JSON.parse(
													styles[li].children[a].attribs['data-images']
												).zoomed_url
										);
									}
								}
...
api.getItem = function (itemURL, callback) {
	request(
		...
		function (err, resp, html, rrr, body) {
            ...
            var addCartButton = $('input[type=submit][name=commit]')[0];
				...
				for (li in styles) {
					for (a in styles[li].children) {
						if (
							...
							metadata.images.push(
								'https:' +
									JSON.parse(styles[li].children[a].attribs['data-images'])
										.zoomed_url
							);
						}
					}
			...
			else if (metadata.title.indexOf('Skateboard') != -1) {
			    metadata.images.push('https:' + $('#img-main').attr('src'))
			}
...
api.seek = function (category, keywords, styleSelection, callback) {
	...
	api.getItems(category, (product, err) => {
		...
		for (i = 0; i < product.length; i++) {
			...
			if (style === null) {
				...
				if (title.indexOf(keywords) > -1) {
					// check if the keywords match with the title
					// found item
		...
		if (productLink[0] === undefined) {
			return callback(
				null,
				'Could not find any results matching your keywords.'
			);
		}
	});
};
